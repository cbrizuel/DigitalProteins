...
class CPD(Problem):
    ...
    def __init__(self,
        ...
        # self.eng_calc = Sigmoid(scaling_factor=0.002, 
        #                         offset=-2.0, 
    ...
    def compute_fitness(self, sequence: npt.NDArray[np.int64]) -> float:
        ...
        ########################################################################
        # eng = self.eng_calc._metric._ref2015(self.prediction_pdb_file_path)
        # eng = self.eng_calc._normalize(eng)
        # cm_rms = self.cm_calc._metric._contact_map_rms(model_bb, 
        #                                                self.ref_backbone)
        # cm_rms = self.cm_calc._normalize(cm_rms)
        # ----------------------------------------------------------------------
        self.superimposer.set_atoms(self.ref_backbone, model_bb)
        rmsd = self.superimposer.apply(model_bb)
        ...
        # return np.array([ cm_rms, cyc, eng, plddt ])
        return np.array([ gdt, cyc, plddt ])
...
class SaveProgression(Callback):
    ...
    def __init__(self,
        ...
        self.term_values = np.full((num_generations, population_size, 3), 
                                   -1,
                                   np.float64)
