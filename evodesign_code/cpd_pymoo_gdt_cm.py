...
class CPD(Problem):
    ...
    def __init__(self,
        ...
        self.cyc_calc = CyclizationNormalized(scaling_factor=0.05)
        self.eng_calc = RosettaEnergyFunction()
        self.gdt_calc = Gdt(rmsd_metric=Rmsd())
        self.tm_calc = TMScore()
    ...
    def compute_fitness(self, sequence: npt.NDArray[np.int64]) -> float:
        ...
        eng = self.eng_calc._ref2015(self.prediction_pdb_file_path)
        ...
        rmsd = self.gdt_calc._rmsd_metric._rmsd(model_bb, self.ref_backbone)
        rmsd = 1 / (1 + rmsd)
        gdt = self.gdt_calc._gdt(model_bb, self.ref_backbone)
        tm = self.tm_calc._tmscore(model_bb, self.ref_backbone)
        return np.array([ rmsd, gdt, tm, cm_rms, cyc, eng, plddt ])
...
class SaveProgression(Callback):
    ...
    def __init__(self,
        ...
        self.term_values = np.full((num_generations, population_size, 7), 
                                   -1,
                                   np.float64)
    ...
    def notify(self, algorithm):
        ...
        if generation_id % self.save_every_generation == 0:
            ...
            temp = algorithm.problem.eng_calc
            algorithm.problem.eng_calc = None
            save_algorithm(algorithm, self.algorithm_file_path)
            algorithm.problem.eng_calc = temp
